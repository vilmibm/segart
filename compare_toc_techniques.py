...
def main():
    print(f"{'item / variant':<60} {'ent':>4} {'xref':>5} {'ILLhit':>7} {'leaf=':>5} {'leaf±':>5} {'title':>5} {'auth':>4}")
    print("-" * 105)
    rows = []
    for item in ITEMS:
        for label, suffix in (("heur", "_toc_heur.json"), ("llm",  "_toc_llm.json")):
    ...
    for kind in ("heur", "llm"):
        ...
        print(f"{kind:>5}: entries={ent}  crossref={xref}  "
              f"ILL hit={hits}/{anch} ({100*hits//max(anch,1)}%)  "
              f"leaf=:{lstrict}/{anch}  leaf±:{lsoft}/{anch}  "
              f"title:{title}/{anch}  auth:{auth}/{anch}")
